        ...
        GuiRebarBlock
{
    ...
    public final ItemStackBuilder zotStack = ItemStackBuilder.gui(Material.PURPLE_STAINED_GLASS_PANE, getKey() + ":zot")
            .name(Component.translatable("rebartranscendence.gui.quirp_annihilator.zot"));
    private final ItemStackBuilder progressItemStackBuilder = ItemStackBuilder.of(Material.CLOCK)
            .name(Component.translatable("rebartranscendence.gui.quirp_annihilator.progress"));
    ...
    public QuirpAnnihilator(@NotNull Block block, @NotNull BlockCreateContext context) {
        super(block, context);
        setTickInterval(20);
    ...
    public QuirpAnnihilator(@NotNull Block block, @NotNull PersistentDataContainer pdc) {
        super(block, pdc);
    }
    @Override
    ...
    public void tick() {
        ...
        if (!outputInventory.canHold(RebarTranscEndenceItems.QUIRP_CONDENSATE.clone())) return;
        ...
        if (zotInventory.containsSimilar(RebarTranscEndenceItems.QUIRP_DOWN.clone()) 
                    && zotInventory.containsSimilar(RebarTranscEndenceItems.QUIRP_UP.clone())) {
        ...
        } else if (zotInventory.containsSimilar(RebarTranscEndenceItems.QUIRP_RIGHT.clone()) 
                    && zotInventory.containsSimilar(RebarTranscEndenceItems.QUIRP_LEFT.clone())) {
            zotInventory.setItem(new MachineUpdateReason(), 0, 
                zotInventory.getItem(0).asQuantity(zotInventory.getItemAmount(0) - 1));
            zotInventory.setItem(new MachineUpdateReason(), 1, 
                zotInventory.getItem(1).asQuantity(zotInventory.getItemAmount(1) - 1));
    ...
    public void onProcessFinished() {
        progressItem.notifyWindows();
        outputInventory.addItem(new MachineUpdateReason(), RebarTranscEndenceItems.QUIRP_CONDENSATE.clone());
    }
    @Override
    ...
    public @Nullable WailaDisplay getWaila(@NotNull Player player) {
        ...
        display.add(isProcessing()
            ? Component.translatable("rebartranscendence.waila.quirp_annihilator.running")
                .arguments(
            ...
            : Component.translatable("rebartranscendence.waila.quirp_annihilator.not_running"));
